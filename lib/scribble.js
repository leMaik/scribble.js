  ...
  scribblejs = function($) {
    ...
    Scribble = (function() {
      function Scribble(el, opts) {
        ...
        painting = false;
        stop = (function(_this) {
          return function(e, tool, oldTool) {
            var undoAction;
            oldTool = oldTool ? $.scribble.tools[oldTool] : currentTool();
            if (oldTool && (painting || oldTool.customStopHandling)) {
              painting = false;
              _this.actions = oldTool.stopUse.call(void 0, _this.canvas[0].getContext('2d'), getCursorPosition(e), _this.actions, _this.scale);
              _this.redraw();
      ...
      Scribble.prototype.set = function(key, value) {
        ...
        this[key] = value;
        if (key === 'tool') {
          this.canvas.trigger('scribble:toolchanged', [value, old]);
        }
        return this.canvas.trigger("sketch.change" + key, [value, old]);
      };
      ...
      Scribble.prototype.redraw = function(predicate) {
        ...
        return this.drawOn(context, this.canvas.width(), this.canvas.height(), predicate);
      };

      Scribble.prototype.drawOn = function(context, width, height, predicate) {
        var actions, newHeight, newWidth, ratio;
        ...
        actions = predicate != null ? this.actions.filter(predicate) : this.actions;
        return Scribble.drawShapesOn(contect, actions, this.scale);
      };

      Scribble.drawShapesOn = function(context, shapes, scale) {
        var action, i, len;
        for (i = 0, len = shapes.length; i < len; i++) {
          action = shapes[i];
          if (action.tool) {
            $.scribble.tools[action.tool].draw.call(void 0, action, context, scale || 1);
          }
        }
      };
